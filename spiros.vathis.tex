...
\section{Professional Experience}
  \resumeSubHeadingListStart
    ...
    \resumeSubheading
      {Cloud Software Group}{Patras, Greece}
      {Principal Software Engineer}{Oct 2023 -- Dec 2025}
      \resumeItemListStart
        \resumeItem{Cloud Migration Architecture}
          {Designed the Azure-based migration architecture for the Business Telemetry platform (ACI networking, private endpoints, secure data-access paths), enabling a scalable and cloud-native deployment model for cross-product telemetry.}

        \resumeItem{Reliability Engineering}
          {Eliminated management CPU spikes through redesigned counter-retrieval logic and replaced long-running ETL daemons with scheduled, immutable container executions, significantly improving platform reliability and operational efficiency.}

        \resumeItem{Observability \& Data Quality}
          {Delivered CCID-aware telemetry pipelines, LUI data integration, active-usage aggregation and the Telemetry Validation Playbook, strengthening system observability and proactive detection of data inconsistencies.}

        \resumeItem{Governance \& CI/CD Maturity}
          {Established full containerization for ETL workloads and introduced SQL-as-code for schema lifecycle management, improving deployment consistency, traceability and integration with CI/CD workflows.}

        \resumeItem{Cross-team Enablement}
